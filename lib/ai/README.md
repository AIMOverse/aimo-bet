...
│   └── prompts/
│       └── tradingPrompt.ts       # System prompt and context builder
...
The trading prompt defines agent behavior:
...
The trading cron (`/api/cron/trading`) runs agents autonomously:

1. Fetch current market prices
2. Detect price swings (5% threshold)
3. For each model with wallet:
   - Create `PredictionMarketAgent` with wallet context
   - Execute agentic trading loop
   - Save trades and reasoning to chat
